...
def p_bracket_sq(grad_f, grad_g, uv):
    """
    grad_f, grad_g are normalized gradients, and are of size num_samples x (2s)
    uv is of size: num_samples x (2s). They are the (unnormalized) coordinates    
    ...
    v = uv[:, s:]
    uv_squared = u**2 + v**2
    const = (C + uv_squared / 2)
    const_normalized = F.normalize(const)
    ...
    sum1 = (dfdq * dgdp * const_normalized).sum(1)
    sum2 = (dfdp * dgdq * const_normalized).sum(1)
    p_bracket = sum1 - sum2
...
def compute_loss_h_all(f, models, inputs, training=True):
    ...
    if num_models == 1:
        ...
        numerator = numerator + p_bracket_sq(grad_H2_normalized, grad_H1_normalized, inputs)
        ...
        for grad_H_fixed_current in grad_H_fixed_list:
            ...
            numerator = numerator + p_bracket_sq(grad_H_last_normalized, grad_H_fixed_current_normalized, inputs)
