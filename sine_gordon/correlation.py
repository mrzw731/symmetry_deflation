...
def f(x):
    ...
    u_dot = v
    v_dot = C * (u_nm1 + u_np1 -2*u) - torch.sin(u)
...
for idx in range(num_Hs):
    ...
    plt.yscale('log')
    plt.title('Sine-Gordon, learning H%d, C = %.1f, Num sites = %d' %(idx+1, C, N_sites))
...
def gt_func_H1(x):
    ...
    q_np1 = torch.cat([q[:, 1:], q[:,0:1]], dim=1)

    sum_1 = .5 * p**2
    sum_2 = N_sites * (q_np1 - q)**2
    sum_3 =  - torch.cos(q)

    return (sum_1 + sum_2 + sum_3).sum(1)


choices = np.random.choice(n_train, batch_size)
inputs = torch.tensor(input_test[choices], requires_grad=True, dtype=torch.float, device=cpu)        
gt_Hs = gt_func_H1(inputs).data.numpy()


learned_Hs = models[0](inputs).data.numpy().reshape(-1)


# calcualting
gt_H_now = gt_Hs
learned_H_now = learned_Hs
fig, ax = plt.subplots()
...
plt.xlabel('Ground-truth conservation law: ', fontsize=14)
plt.ylabel('Learned conservation law: ', fontsize=14)
