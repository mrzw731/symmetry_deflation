...
epochs_list = [500, 500, 1000]             # num of epochs to learn each H
...
def f(x):
    ...
    # Periodic
    u_dot = v
    v_dot = torch.cat([torch.exp(-(u[:,0:1]-u[:,-1:]))-torch.exp(-(u[:,1:2]-u[:,0:1])),
                       torch.exp(-(u[:,1:-1]-u[:,0:-2]))-torch.exp(-(u[:,2:]-u[:,1:-1])),
                       torch.exp(-(u[:,-1:]-u[:,-2:-1]))-torch.exp(-(u[:,0:1]-u[:,-1:]))], dim=1)
