...
for N_sites in range(3, 11):
    test_loss_fput_small_deflate=np.load('results_box_50/involution_N_sites=%d_box=50.0_deflate=0.5/losses_test_all.npy' % N_sites)
    test_loss_fput_large_deflate=np.load('results_box_50/involution_N_sites=%d_box=50.0_deflate=1.0/losses_test_all.npy' % N_sites)    
    ...
    plt.xticks(x)
    plt.savefig('test_error_box_50_fput_al_Nsites_%d.png'% N_sites)
    plt.close()
