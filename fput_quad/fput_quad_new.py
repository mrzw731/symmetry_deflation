...
def f(x):
    ...
    # Periodic
    u_dot = v
    v_dot = torch.cat([-(u[:,0:1]-u[:,-1:]) + (u[:,0:1]-u[:,-1:])**2/2 - (-(u[:,1:2]-u[:,0:1]) + (u[:,1:2]-u[:,0:1])**2/2),
                       -(u[:,1:-1]-u[:,0:-2]) + (u[:,1:-1]-u[:,0:-2])**2/2 - (-(u[:,2:]-u[:,1:-1]) + (u[:,2:]-u[:,1:-1])**2/2),
                       -(u[:,-1:]-u[:,-2:-1]) + (u[:,-1:]-u[:,-2:-1])**2/2 - (-(u[:,0:1]-u[:,-1:]) + (u[:,0:1]-u[:,-1:])**2/2)
                       ], dim=1)
...
for idx in range(num_Hs):
    ...
    plt.yscale('log')
    plt.title('FPUT Quadratic, learning H%d, C = %.1f, Num sites = %d' %(idx+1, C, N_sites))
